...
Since the ability of a user to sign up, login, and add a character to an account had been implemented in Sprint 1, for this sprint the focus was primarily on administrator functionality, though some user functionality was also updated. First the abilities for a user to view all the characters on their account by sending a POST request to the backend (characters/get) and to delete characters from their account by sending a DELETE request to the backend (characters/delete) were implemented. Afterward, functionality was implemented to determine if a user was an administrator or not upon logging in. Then administrative abilities of viewing, creating, and deleting spells and items were implemented, allowing a user to add and delete items and spells only if they were logged in as an administrator. Lastly testing was set up to ensure these methods worked correctly and connected to the database properly.
